...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Proactively wake up the backend on Render free tier
    wakeBackend().finally(() => {
      setIsWaking(false);
    });
  }, []);
        ...
        <Card variant="glass" className="border-white/10 shadow-2xl backdrop-blur-xl">
          ...
          <CardContent className="pb-8 pt-6">
            <Button
              ...
              size="lg"
              disabled={isWaking}
              className="w-full flex items-center justify-center gap-3 h-12 text-foreground hover:bg-muted disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {isWaking ? (
                <>
                  <div className="h-5 w-5 animate-spin rounded-full border-2 border-primary border-t-transparent" />
                  <span>Server warming up, please wait…</span>
                </>
              ) : (
                <>
